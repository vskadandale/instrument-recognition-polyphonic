...
import sys
sys.path.append('..')
from data_prepar import multi_label_stratif
import numpy as np
import pandas as pd
...
The first four sets are considered as training set and the last one is considered as test set.
The data from these sets are stored in the form of chunks of 100 samples in each csv. We had to split the data into chunks
because we faced 'Out of Memory' issues while loading them when we persisted all of them in .pkl, .npy or .mat formats. 
This code has been largely borrowed from https://github.com/glennq/instrument-recognition/blob/master/data/gen_split.py
"""
...
def read_txt_file(fname, fpath=os.curdir):
    ...
    """
    with open(os.path.join(fpath, fname), 'rb') as f:
        file_data = f.read().decode().strip().split('\n')
    return file_data
...
def build_train_test_set(datasets, all_instruments, rare_instr, conf):
    ...
    for k, elm in enumerate(datasets):
        ...
        # build test set
        for i in elm:
            data_read = loadmat(os.path.join(matfile_path,'{}_patched.mat'.format(i)))
            for j, e in enumerate(['X', 'y', 'present', 'song_name', 'time']):
                if e=='X':
                    data[j].append(data_read[e][:, None])
                else:
        ...
        """
        chunk_size=100     
        number_of_chunks=math.floor(data['X'].shape[0]/chunk_size)
        for key in data.keys():
            chunks=np.vsplit(data[key][:number_of_chunks*chunk_size],number_of_chunks)
            chunks.append(data[key][number_of_chunks*chunk_size:])
            for i in range(len(chunks)):
                dataset_part=os.path.join(dataset_path,'dataset_{}/{}'.format(k,key))
                create_folder(dataset_part)
                pd.DataFrame(chunks[i]).to_csv(os.path.join(dataset_part,'{}.csv'.format(i)),index=False, header=False)

        del data
        print('finished building data set {}'.format(k))
        """
        create_folder(dataset_path)
        dataset_part = os.path.join(dataset_path, 'dataset_{}'.format(k))
        np.save(dataset_part,data)
        print('finished building data set {}'.format(k))

def generate_split(groupID, rare_thres, conf, songs_to_split=None, start_song=0,
    ...
    #SAVE THE SPLITS
    #with open('dataset_splits.pkl', 'wb') as fp:
    ...
    for idx, e in enumerate(datasets):
        ...
        with open('dataset_{}_instr_distr.txt'.format(idx), 'wb') as f:
            f.write('\n'.join(dataset_cnt).encode())
    build_train_test_set(datasets, all_instruments, rare_instr, conf)
...
def main():
    ...
    #generate_split(4, 20, conf=args.config, num_split=10, p_split=list(np.ones(10) * 0.1),
    generate_split(4, 20, conf=args.config, num_split=5, p_split=list(np.ones(5) * 0.2),
                   rand_state=2345)
