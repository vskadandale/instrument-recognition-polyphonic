...
class Trainer(object):
    ...
    def _load_features(self, filenames):
        ...
        for filename in filenames:
            feature_filename = os.path.join(MEDLEY_TRAIN_FEATURE_BASEPATH, self.config,self.data_type,self.channel,
                                            "{}.npy".format(filename))
...
def main():
    ...
    print "TEST TRAIN SPLIT SUCCESS!!!"
    ...
    filenames=np.load(os.path.join(PATH_TO_METADATA,args.config,args.data_type,args.channel,'medley_train_filenames.npy'))
    #labels=np.load('/homedtic/vshenoykadandale/DeepLearning/metadata/medley_train_labels_bin.npy')
    raw_labels=np.load(os.path.join(PATH_TO_METADATA,args.config,args.data_type,args.channel,'medley_train_labels.npy'))
