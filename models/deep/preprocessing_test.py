...
"""
This code extracts the mel-spectrograms from the audio belonging to test set and stores them. 
...
def preprocess(model_module,config,channel,data_type):
    path_to_dataset=os.path.join(PATH_TO_WAV_FILES,data_type,'wavs_'+config)
    path_to_features=os.path.join(MEDLEY_TEST_FEATURE_BASEPATH,config,data_type,channel)
    ...
    for dataset in datasets:
        ...
        for sub_folder in sub_folders:
            ...
            for filename in files:
                ...
                for i, spec_segment in enumerate(model_module.compute_spectrograms(os.path.join(sub_folder_path, filename))):
                    feature_filename = os.path.join(path_to_features,
                            "{f_name}_{segment_idx}".format(f_name=f_name,segment_idx=i))
                    if(start):
            ...
            del temp_df
    np.save(os.path.join(path_to_metadata,'medley_test_filenames'),list_of_files)
    np.save(os.path.join(path_to_metadata,'medley_test_labels'),labels)
...
def main():
    ...
    try:
        ...
        print e
    preprocess(model_module,args.config,args.channel,args.data_type)
