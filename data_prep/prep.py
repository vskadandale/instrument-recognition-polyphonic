...
def normalize_data(data):
    ...
    """
    for k in data.keys():
        #For 16-bit quantized audio
        data[k] = np.float32(data[k]/32768)
...
def split_music_to_patches(data, conf, annotation, inst_map, label_aggr, length=1,
    ...
    Args:
        ...
        conf (str): the config of data. For ex: _1s_h100 means 1s long window with full hop
...
def prep_data(in_path, conf, out_path=os.curdir, save_size=20, norm_channel=False,
    ...
    for i in range(max(start_from, 0), len(dlist), save_size):
        ...
        for k, v in data.items():
            ...
            patches_save_path = os.path.join(matfile_path,'{}_patched.mat'.format(k))
